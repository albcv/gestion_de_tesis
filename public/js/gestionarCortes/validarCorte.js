document.addEventListener('DOMContentLoaded', function() {
    const formulario = document.getElementById('formulario_corte');
    
    if (formulario) {
        formulario.addEventListener('submit', function(event) {
            const errores = validarFormularioCorte();
...
function validarFormularioCorte() {
    ...
    // Validar número de corte
    const numeroCorteSelect = document.getElementById('número_corte');
    const numeroCorteError = validarSelect(numeroCorteSelect, 'Número de corte');
    if (numeroCorteError) errores.push(numeroCorteError);
    
    // Validar enlace (si está presente)
    const enlaceInput = document.getElementById('enlace');
    if (enlaceInput.value.trim() !== '') {
        const enlaceError = validarURL(enlaceInput.value);
        if (enlaceError) errores.push(enlaceError);
    }
...
function validarURL(url) {
    try {
        ...
        return 'El enlace a GitHub debe ser una URL válida';
    }
}
...
function configurarValidacionEnTiempoReal() {
    ...
    if (numeroCorteSelect) {
        numeroCorteSelect.addEventListener('blur', function() {
            const error = validarSelect(this, 'Número de corte');
            mostrarErrorCampo(this, error);
        });
    }
    ...
    if (enlaceInput) {
        enlaceInput.addEventListener('blur', function() {
            if (this.value.trim() !== '') {
                const error = validarURL(this.value);
                mostrarErrorCampo(this, error);
            } else {
                mostrarErrorCampo(this, null);
            }
        });
    }
