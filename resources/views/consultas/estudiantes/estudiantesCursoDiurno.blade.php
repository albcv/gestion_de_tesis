...
@if(isset($estudiantes))
    @if (count($estudiantes) != 0)
        <div class="resultado-info">
            <p style="text-align: center;">Total de estudiantes del curso diurno encontrados: {{ count($estudiantes) }}</p>

            {{-- Botón Exportar CSV --}}
            <div class="acciones-exportar">
                <a href="{{ route('exportarConsultaCsv', array_merge(request()->query(), ['tipo' => 'curso_diurno'])) }}"
    ...
    @else
        <script>
            @if(isset($carreraParam) && $carreraParam)
                alert('No hay estudiantes del curso diurno en esa carrera');
            @endif
        </script>
    @endif
@endif
